...
namespace MinecraftClone
{
  namespace Cubes
  {
    ...
    void drawCube(const glm::vec3 &position, const Texture &texture, Shader shader)
    {
    ...
    void init(Shader shader)
    {
      ...
      float windowAspect = ((float)1280 / (float)720);
      float fov = 70.0f;
      float zNear = 0.1f;
      float zFar = 10'000.0f;
      projection = glm::perspective(fov, windowAspect, zNear, zFar);

      // for (auto &filepath : std::filesystem::directory_iterator("extras/textures"))
      // {
      ...
      normalTexture = createTexture("extras/textures/normal.jpg");
    }
    ...
    void update(float dt, Shader shader, glm::mat4 view, glm::mat4 proj)
    {
      ...
      shader.SetUniformMat4f("uProjection", proj);
      ...
      shader.SetUniform1i("uNormalTexture", 1);

      for (int i = 0; i < cubePositions.size(); i++)
      {
        drawCube(cubePositions[i], textures.at(cubeTextures.at(i)), shader);
      }
    }
  }
}